...
class TestDetectTextRotation:
    ...
    def test_vertical(self):
        # 两步法默认（must_rotate=False）：右面可读 → 0°；左面 → 180°
        assert detect_text_rotation(text_data((0.0, -1.0))) == 0
        assert detect_text_rotation(text_data((0.0, 1.0))) == 180

    def test_no_text(self):
        assert detect_text_rotation({"blocks": []}) == 0  # 无文字默认基础旋转 0
    ...
    def test_weighted_dominant_vertical(self):
        # 左面（0,1）占主导 → 两步法默认 must_rotate=False → 180°
        data = {
            "blocks": [
...
class TestPlanRotation:
    ...
    def test_a3_landscape_no_rotation(self):
        ...
        r, size = plan_rotation(p, None)
        assert r == 0
        assert size == (A3_HEIGHT_MM, A3_WIDTH_MM)  # 原横向尺寸，0° 不交换
